...
plot(malawi$KS_nd, malawi$cd4under200)
...
#labels = c("low","high")
malawi$CD400low400 <- cut(malawi$cd4count, ordered_result = F, breaks = c(0,400,600), labels = c("yes","no"))
...
# SIMPLE 
malawi.lm <- lm(logHIV ~ cd4count, data = subset(malawi, malawi$cd4count < 600))
summary(malawi.lm)
malawi.lm <- rlm(logHIV ~ cd4count, data = subset(malawi, malawi$cd4count < 600))
summary(malawi.lm)


# VARIANT 1:		Using the parameters of Mellors 1996: log2
#
malawi.Mellors <- lm(log2HIV ~ cd4count, data = subset(malawi, malawi$cd4count < 600))
summary(malawi.Mellors)
malawi.Mellors <- rlm(logHIV ~ cd4count, data = subset(malawi, malawi$cd4count < 600))
summary(malawi.Mellors)

# VARIANT2: 		VARIANT 1 & Restrict to CD4 < 600
#
malawiLowCD4 <- subset(malawi, malawi$cd4count < 600)
malawiLowCD4 <- na.omit(malawiLowCD4)
malawiLowCD4 <- drop.levels(malawiLowCD4)
summary(malawiLowCD4)
cor(malawiLowCD4$log2HIV, malawiLowCD4$cd4count, method = "spearman")
cor.test(malawiLowCD4$log2HIV, malawiLowCD4$cd4count, method = "spearman")

# VARIANT 3:		now using sqrt(CD4 count)
cor(malawiLowCD4$log2HIV, malawiLowCD4$sqrtcd4, method = "spearman")
cor.test(malawiLowCD4$log2HIV, malawiLowCD4$sqrtcd4, method = "spearman")
...
plot(malawiLowCD4$sqrtcd4,malawiLowCD4$log2HIV, col = "darkgray", pch = 16, main = "", xlab = "sqrt(CD4 cells / µl)", ylab = "lg HIV cps/ml",  cex =1.4)
...
png(file = "VL.png", units = "in", width = 8, height = 10, res = 600)
{
attach(malawi)
...
png(file = "PCAvar.png", units = "in", width = 10, height = 8, res = 600)
{
par(mfrow = c(2,2))
colnames(malawi)
malawi.colorcode <- 11
...
dimdesc(malawi.pca)
barplot(malawi.pca$eig[,1], main = "Eigenvalues", names.arg = paste ("Dim", 1:nrow(malawi.pca$eig), sep = ""))
...
png(file = "TREE_lyticscorefactor.png", units = "in", width = 6, height = 10, res = 600)
{
par(mfrow=c(1,1))
plot(malawi.rpart)
...
png(file = "TREE.png", units = "in", width = 6, height = 10, res = 600)
{
plot(malawi.rpart3)
text(malawi.rpart3, use.n=TRUE, cex = 1.5, col = "darkblue")
}
dev.off()
