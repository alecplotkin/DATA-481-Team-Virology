...
```{r}
# -----------------------------------------------------------------------------------------------------
# SCREEN: Density plot of real primers and NTC ----
#
colnames(rain.wide)
par(mfrow = c(1,1))
par(bty = "n")
...
mir.pca <- PCA(for.pca, scale.unit = T, ncp = 5, graph = T)
...
```{r  PCA cluster tree,include=TRUE}

# -----------------------------------------------------------------------------------------------------
# STEP:  PCA  & cluster tree
#
mir.hcpc <- HCPC(mir.pca, nb.clust = -1, graph = T) # edited
mir.hcpc$desc.var
mir.hcpc$desc.axe
mir.hcpc$desc.ind
print(treeheight)
...
```{r  PCA cluster tree A,include=TRUE}
...
```{r  PCA cluster tree C,include=TRUE}
mir.hcpc <- HCPC(mir.pca, nb.clust = -1, graph = T)
mir.hcpc$desc.var
mir.hcpc$desc.axe
